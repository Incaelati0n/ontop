...
/**
 *  Flink-SQL functions description available at : https://ci.apache.org/projects/flink/flink-docs-stable/
 */
public class FlinkSQLDBFunctionSymbolFactory extends AbstractSQLDBFunctionSymbolFactory {

    private static final String UNSUPPORTED_MSG = "Not supported by Flink or not still to be implemented";
    ...
    protected String serializeDateTimeNorm(ImmutableList<? extends ImmutableTerm> terms,
        ...
        return String.format("REPLACE(DATE_FORMAT(%s,'yyyy-MM-dd HH:mm:ss.SSS'),' ','T')", termConverter.apply(terms.get(0)));
    }

    @Override
    protected DBTypeConversionFunctionSymbol createDateTimeDenormFunctionSymbol(DBTermType timestampType) {
        return new FlinkSQLTimestampDenormFunctionSymbol(timestampType, dbStringType);
    }
